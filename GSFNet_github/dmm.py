...
class StackedHourglass(keras.Model):
    ...
    def __init__(self, filters):
        ...
        self.conv_layer = tf.keras.layers.Conv3D(filters=1, kernel_size=(48, 6, 6), strides=(8, 8, 8), padding='valid')

    def call(self, inputs,attention, training=None, mask=None):
...
class feature_shared(keras.Model):
    def __init__(self, filters):
        super(feature_shared, self).__init__()
    ...
    def call(self, inputs):
        ...
        x=self.trans2 (inputs)
        return x


class PSM(keras.Model):
    ...
    def __init__(self, filters, max_disp):
        ...
        #self.extractor = FeatureExtractor( filters=2*self.filters)
        self.extractor = feature_shared(filters= self.filters)
        self.constructor = CostVolume(max_disp=self.max_disp // 4)
        self.hourglass = StackedHourglass( filters=2*self.filters)
        self.estimation = Estimation(max_disp=self.max_disp)
    ...
    def call(self,inputs):
        ...
        left_feature =  self.extractor(inputs[0])
        right_feature =  self.extractor(inputs[1])

        # cost volume
        
        cost_volume = self.constructor([left_feature, right_feature])

        # 3D CNN (stacked hourglass)
        
        [out1, out2, out3] = self.hourglass(cost_volume,inputs[2])

        # disparity

        d3 = self.estimation(out3)
